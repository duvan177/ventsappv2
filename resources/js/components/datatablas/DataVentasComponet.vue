    ...
    <v-container max-width="100% !imporant">
      <v-card>
        <v-card-subtitle style="background:#1565c0;color: white;">
          <h2>
            ventas
            <v-icon size="45" color="white">mdi-cart-plus</v-icon>
          </h2>
          <v-spacer></v-spacer>
          <!-- <v-col cols="6" sm="6">
          ...
          <div justify-center class="row">
            <v-col class cols="12" md="3">
              <v-card color="primary" height="130%">
                <v-toolbar-title
                  style="color: white;"
                  v-text="'Total ventas: $'+eventoNum(totalingreso)"
                ></v-toolbar-title>
              </v-card>
            </v-col>
            <v-col cols="12" md="3">
              <v-card color="primary" height="130%">
                <v-toolbar-title
                  style="color: white;"
                  v-text="'total rentabilidad: $'+rentab"
                ></v-toolbar-title>
              </v-card>
            </v-col>
              <v-col cols="12" md="3">
        ...
        <v-card-text>
          ...
          >
            ...
            <template v-slot:item.cliente="{ item }">{{item.cliente.nombre}}</template>
            <template v-slot:item.name="{ item }">{{item.usuario.name}}</template>
                <template v-slot:item.nombreart="{ item }">{{item.articulo.nombre}}</template>
             <template v-slot:item.precio_venta_art="{ item }">{{item.articulo.precio_venta_art}}</template>
            
            <template v-slot:item.totalventa="{ item }">{{showtotal(item)}}</template>
             <template v-slot:item.estado="{ item }">
             <v-chip :color="getColor(item.estado)" dark>{{ estadoventa(item.estado) }}</v-chip>
            </template>
                     <template v-slot:item.renta="{ item }">
             <v-chip :color="getColor(item.estado)" class="white--text"  >{{ '+'+ rentabilidad(item) }}</v-chip>
            </template>
          ...
          <v-card-actions>
            <div class="row">
              <v-col cols="12" md="4">
                <v-toolbar-title v-text="'Total ventas: $'+eventoNum(totalingreso)"></v-toolbar-title>
              </v-col>
              <v-col cols="12" md="4">
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "precio", value: "precio_venta_art" },
      { text: "total", align: "right", value: "totalventa" },
        { text: "rentabilidad", align: "right", value: "renta" },
      { text: "Impuesto", align: "right", value: "impuesto" },
      { text: "Estado", value: "estado" },
      { text: "fecha de salidad", value: "created_at" }
  ...
  methods: {
    ...
    showtotal(obj) {
      let total = 0;
      total = parseInt(obj.precio_venta) * obj.cantidad;
      return total;
    },
    eventoNum(i) {
